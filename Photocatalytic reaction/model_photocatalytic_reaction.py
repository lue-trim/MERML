...
def EP_plot(true, pred, name):
    ...
    ax.text(x=65, y=-5, s='RMSE = ' + str(np.round(np.sqrt(mean_squared_error(true, pred)), 3)),
            fontdict={'family': 'Arial', 'size': '18', 'weight': '500'})
    plt.savefig('/home/huatianwei/Code/Code for MEARML model/Figures/'+name+'.svg', dpi=600)
    # plt.show()


def model_train(train_data, params, num, name, lag=2, shifts=[1],):
    train = []
    time_single = [0, 10, 30, 60, 100, 150, 210]
    ...
    EP_plot(train[:, -1]*100, y_pred*100, name)
...
def model_test(model_trained, test_data, name, lag=2, shifts=[1]):
    ...
    EP_plot(true * 100, pred * 100, name)


if __name__ == '__main__':
    x = np.load('x_photocatalytic.npy')
    y = np.load('y_photocatalytic.npy')
    ...
    params = {'eta': 0.02, 'max_depth': 7, 'min_child_weight': 4}
    # params = {'eta': 0.1, 'max_depth': 4, 'min_child_weight': 4}
    model_trained = model_train(data_train, params, num=200, name='Simulated Second Train')
    # model_trained = load('MCB')
    model_test(model_trained, data_test, name='Simulated Second Test')